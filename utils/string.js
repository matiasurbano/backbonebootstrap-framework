...
// string.format('hi {1}, {3} and {2}', 'a', 'b', 'c') ==>
// "hi a, c and b"
...
string.stripFirst = function(text, prefix) {
  return string.startsWith(text, prefix) ? text.substr(prefix.length) : text;
};

string.stripLast = function(text, suffix) {
  return string.endsWith(text, suffix) ?
    text.substr(0, text.length - suffix.length) : text;
};
