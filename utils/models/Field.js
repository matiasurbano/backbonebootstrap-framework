...
var Field = BaseObject.extend({
  ...
  supportedTypes: ['number', 'string', 'date', 'boolean', 'object', 'check', 'arrayobject'],
  ...
  initialize: function(options) {
    ...
    // used for specifing field to filter
    // Object field should overwrite it
    if (options.filterBy) this.filterBy = options.filterBy;
    ...
    if (_.isString(this.control)) this.control = { type: this.control };
    else if (!this.control)       this.control = { type: 'input' };
  ...
  initControl: function() {
    if (this.control instanceof BaseControl) return this.control;

    this.control.field = this;
    this.control = controlsHelper.createControl(this.control);
    return this.control;
  },
  ...
  /**
   * This function / string should return the string to add to the filterBy
   * param to filter by the contents of this field.
   *
   * Calculated fields (like foreign keys) should overwrite this to add all
   * the fields involved in the calculation.
   *
   * Example:
   *
   * display           : function(zona) {
   *   return zona.Descripcion + ' (' + zona.Codigo + ')';
   * },
   * filterBy          : 'Zona.Descripcion,Zona.Codigo',
   *
   * The previus field specifies a calculated expression, so it has to pass
   * the involved fields when filtering by that parameter.
   *
