...
convert.toDate = function(stringDate, format) {
  ...
  if (format.toLowerCase() === 'asp.net') {
    // moment.js can handle asp.net dates by default
    // we specify no format
    momentDate = moment(stringDate);
  } else {
    momentDate = moment(stringDate, format);
  }
...
convert.formatDate = function(rawDate, format) {
  // #TODO hacer esto en AppBaseCollection - date handling
  // ASP no puede parsear '' como fecha, necesita null
  ...
  // handle special cases
  if (format.toLowerCase() === 'asp.net') {
    //#TODO: verify how to represent an empty date in asp.net
    return '/Date(' + momentDate.valueOf().toString() + ')/';
  }
