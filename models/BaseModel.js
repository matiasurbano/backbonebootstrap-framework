 ...
 *
 * If has a FieldCollection with all the information necessary to process the
 * fields.
 *
...
var BaseModel = Backbone.Model.extend({
  ...
  save: function(attributes, options) {
    ...
    // the model must be validated by Default.
    ...
    if (options.validate && _.isFunction(this.validate) && this.validate()) return;
 ...
 */
  ...
  validateField: function(field) {
    ...
    if (!_.findWhere(this.fields, { name: field.name })) {
      throw new Error('Could not find a field with name "' + field + '".');
    }
  ...
  displayAttrs: function(fields, escaped) {
  ...
  /**
   * Initiate each field collection with the corresponding schema definition.
   *
   * It takes this.schema and this.xxxSchema and instantiates the corresponding
   *
   * this.fields and this.xxxFields.
   *
   ...
   */
  initFields: function() {
    var schemaProp, fieldsProp, schema;

    for(schemaProp in this) {

      // only process xxxSchema properties
      if (schemaProp === 'schema') {
        fieldsProp = 'fields';
      } else if (string.endsWith(schemaProp, 'Schema')) {
        fieldsProp = schemaProp.replace('Schema', 'Fields');
      } else {
        continue;
      }
      ...
      // it should be a plain object - skip setSchema methods and similar stuff
      if (!_.isArray(schema)) continue;

      // finally process this field
      this[fieldsProp] = new FieldCollection(schema);
      this[fieldsProp].setModel(this);
    }
    return this;
  },
...
BaseModel.extend = function() {
  ...
  modelHelper.extendSchemas(ExtendedModel);
